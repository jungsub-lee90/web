...
def create(question_id):
    ...
    if form.validate_on_submit():
        ...
        sql = "insert into answer (question_id, content, create_date, user_id) values ({},'{}','{}', {})".format(question_id,
         content, datetime.now(), g.user['user_id'])
        cursor.execute(sql)
        db.commit()
    ...
    ## 답변들 가져오기
    sql = 'select * from answer where question_id = {} order by create_date desc'.format(question_id)
    cursor.execute(sql)
